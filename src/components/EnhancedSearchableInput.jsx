...
// // Add this toast component at the top of your file
// const Toast = ({ message, onClose, type = 'warning' }) => {
//   useEffect(() => {
...
//       type === 'warning' ? 'bg-orange-100 border-orange-400 text-orange-700' : 'bg-red-100 border-red-400 text-red-700'
//     } border px-4 py-3 rounded-lg shadow-lg flex items-center space-x-3 max-w-md`}>
//       <span className="text-2xl">⚠️</span>
//       <div className="flex-1">
//         <p className="font-medium">{message}</p>
//         <p className="text-sm mt-1 opacity-75">This might affect prediction accuracy</p>
//       </div>
//       <button
//         onClick={onClose}
//         className="ml-4 text-2xl hover:opacity-75 transition-opacity"
//         aria-label="Close"
//       >
//         ×
//       </button>
...
//   // Check if selection is a profession mismatch and show warning
//   const checkProfessionMismatch = (person, selectedName) => {
//     const expectedProfession = getExpectedProfession(step);
//     if (!expectedProfession) return;

//     // If we have department info
//     if (person.known_for_department) {
//       const knownFor = person.known_for_department.toLowerCase();
//       const expected = expectedProfession.toLowerCase();
...
//       if (expected === "directing" && knownFor !== "directing") {
//         showWarning = true;
//         message = `${selectedName} is primarily known as ${getProfessionName(knownFor)}, not as a Director.`;
//       } else if (expected === "acting" && knownFor !== "acting") {
//         // Don't warn if it's a singer who also acts
//         if (!(knownFor === "sound" && checkActingCredits(person))) {
//           showWarning = true;
//           message = `${selectedName} is primarily known as ${getProfessionName(knownFor)}, not as an Actor.`;
//         }
//       } else if (expected === "sound" && knownFor !== "sound") {
//         // Don't warn if it's an actor who also sings
//         if (!(knownFor === "acting" && checkMusicCredits(person))) {
//           showWarning = true;
//           message = `${selectedName} is primarily known as ${getProfessionName(knownFor)}, not as a ${step}.`;
//         }
//       }
...
//       // Try to detect mismatch from known_for text
//       const knownForStr = Array.isArray(person.known_for) ? person.known_for.join(', ') : person.known_for;
//       if (typeof knownForStr === 'string') {
//         const lowerKnownFor = knownForStr.toLowerCase();
        
//         // Check if known_for indicates a different profession
//         if (step === "Director" && lowerKnownFor.includes('sound')) {
//           setToastMessage(`${selectedName} appears to be known for Sound/Music work, not as a Director.`);
//           setShowToast(true);
//         } else if (step.includes("Cast") && lowerKnownFor.includes('sound')) {
//           setToastMessage(`${selectedName} appears to be known for Sound/Music work, not as an Actor.`);
//           setShowToast(true);
//         }
//       }
//     }
...
//   // Helper to get readable profession name
//   const getProfessionName = (department) => {
//     switch(department.toLowerCase()) {
//       case 'acting': return 'an Actor';
//       case 'directing': return 'a Director';
//       case 'sound': return 'a Music/Sound professional';
//       case 'production': return 'a Producer';
//       case 'writing': return 'a Writer';
//       default: return `a ${department} professional`;
//     }
//   };
...
//   useEffect(() => {
//     if (popularOptions[step]) {
//       const expectedDept = getExpectedProfession(step);
//       setPopularSuggestions(
//         popularOptions[step].map(name => {
//           // Add known profession data for popular suggestions
//           let department = expectedDept;
          
//           // Override for known cross-talent individuals
//           if (step === "Director" && ["Shah Rukh Khan", "Aamir Khan"].includes(name)) {
//             department = "Acting";
//           } else if (step.includes("Cast") && ["A.R. Rahman", "Diljit Dosanjh"].includes(name)) {
//             department = "Sound";
//           }
...
//           // Add type and clean up results
//           results = results.map(person => {
//             // Extract profession from known_for if it exists
//             let department = person.known_for_department;
            
//             // If known_for contains profession info, try to extract it
//             if (!department && person.known_for) {
//               const knownForStr = Array.isArray(person.known_for) ? person.known_for.join(', ') : person.known_for;
//               if (typeof knownForStr === 'string') {
//                 const lowerKnownFor = knownForStr.toLowerCase();
//                 if (lowerKnownFor.includes('sound') || lowerKnownFor.includes('music')) {
//                   department = 'Sound';
//                 } else if (lowerKnownFor.includes('direct')) {
//                   department = 'Directing';
//                 } else if (lowerKnownFor.includes('act')) {
//                   department = 'Acting';
//                 }
//               }
//             }
...
//     console.log("Known for department:", person.known_for_department);
//     console.log("Known for:", person.known_for);
    
//     // Check for profession mismatch and show warning if needed
//     if (person && typeof person === 'object') {
//       checkProfessionMismatch(person, selectedName);
//     }
...
//         <button 
//           onClick={() => {
//             setToastMessage("Test: Shah Rukh Khan is primarily known as an Actor, not as a Director.");
//             setShowToast(true);
//           }}
//           className="absolute -top-8 right-0 text-xs bg-gray-200 px-2 py-1 rounded"
//         >
//           Test Toast
//         </button>
//       )}
...
//                         {person.known_for && (
//                           <div className="text-xs text-gray-500 truncate">
//                             Known for: {Array.isArray(person.known_for) 
...
function EnhancedSearchableInput({ step, value, onChange, onSelect, placeholder }) {
  const [searchQuery, setSearchQuery] = useState("");
  const [suggestions, setSuggestions] = useState([]);
  const [popularSuggestions, setPopularSuggestions] = useState([]);
  const [isLoading, setIsLoading] = useState(false);
  ...
  // FIXED: Sync searchQuery with value prop when it changes (including reset to empty)
  useEffect(() => {
    // Always sync with the value prop, including when it's empty (reset case)
    setSearchQuery(value || "");
  }, [value]);
